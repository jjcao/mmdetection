...
from ..iou_calculators import kpt_oks
...
class MaxOKSAssigner(BaseAssigner):
    ...
    def assign(self, kpts, gt_kpts, gt_labels=None, img_meta=None):
        """Assign gt to bboxes.
        ...
        if assign_on_cpu:
            device = kpts.device
            bboxes = kpts.cpu()
            gt_bboxes = gt_kpts.cpu()
            if gt_labels is not None:
                gt_labels = gt_labels.cpu()

        kpts = kpts[:, :34]
        overlaps = kpt_oks(gt_kpts, kpts, img_meta)
